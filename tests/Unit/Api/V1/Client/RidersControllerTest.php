...
class RidersControllerTest extends TestCase
{
	...
	// Client'suser should be able to get list of riders
	public function testIndex()
	{
		...
		$this->actingAs($this->clientUser, 'api')
			 ->get('/api/v1/client/riders?per_page=9999')
			 ->assertStatus(200)
			 ->assertJson(['data' => $data]);
	}

	// Client'suser should be able to get details of a rider
	public function testGet()
	{
		...
		foreach ($this->client->fitters as $fitter) {
			$fitters[] = [
				'id'         => $fitter->id,
				'name'       => $fitter->name,
				'abn'        => $fitter->abn,
		...
		$this->actingAs($this->clientUser, 'api')
			 ->get('/api/v1/client/riders/' . $rider->id)
			 ->assertStatus(200)
			 ->assertJson(json_decode(json_encode($data), true));
	}

	// Client'suser should be able to create a rider
	public function testPost()
	{
		...
		$this->actingAs($this->clientUser, 'api')
			 ->post('/api/v1/client/riders', $data)
			 ->assertStatus(200)
			 ->assertJsonStructure([
				 'success',
				 'id',
	...
	// Client'suser should be able to update a rider
	public function testPut()
	{
		...
		$this->actingAs($this->clientUser, 'api')
			 ->put('/api/v1/client/riders/' . $rider->id, $data)
			 ->assertStatus(200)
			 ->assertJsonStructure([
				 'success',
				 'id',
	...
	// Client'suser should be able to delete a rider
	public function testDelete()
	{
		$rider = $this->riders[0];

		$this->actingAs($this->clientUser, 'api')
			 ->delete('/api/v1/client/riders/' . $rider->id)
			 ->assertStatus(200);
