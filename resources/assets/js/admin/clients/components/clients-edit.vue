...
<script>
    export default {
        ...
        methods: {
            postPut() {
                this.myAxios.put(this.clientsResourceUrl + '/' + this.$route.params.id, this.data)
                    .then((response) => {
                        this.showElementNotify('Client Updated', 'Client has updated.');
            ...
            getClient() {
                this.myAxios.get(this.clientsResourceUrl + '/' + this.$route.params.id)
                    .then((response) => {
                        this.data = response.data.client;
                    })
                    .catch((error) => {
                        console.log(error);
                        this.showErrorNotify(error);
