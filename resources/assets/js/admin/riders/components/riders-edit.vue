<template>
    <div class="ibox float-e-margins">
        <div class="ibox-title">
            <h5>Edit Rider: {{ data.first_name }} {{ data.last_name }}</h5>
        </div>
        <div class="ibox-content" v-loading="XHRCount">
            <form class="form-horizontal">

                <div class="form-group" :class="{'has-error': errors.has('client')}">
                ...
                <div class="form-group" :class="{'has-error': errors.has('postcode')}">
                    ...
                    <div class="col-md-10">
                        <input class="form-control" v-model="data.postcode" id="postcode"
                               @focus="showTooltip" @blur="hideTooltip">
                        <div class="tooltip-under-the-box">The postcode must be 4 digits</div>
                ...
                <div class="form-group" :class="{'has-error': errors.has('mobile')}">
                    ...
                    <div class="col-md-10">
                        <input class="form-control no-spin" type="number" v-model="data.mobile" id="mobile"
                               @focus="showTooltip" @blur="hideTooltip">
                        <div class="tooltip-under-the-box">The mobile must be 10 or 11 digits only</div>
                ...
                <div class="form-group" :class="{'has-error': errors.has('has_registered_on_website')}">
                    <label class="col-md-2 control-label"
                           for="has_registered_on_website">Has Registered on Website?</label>
                    <div class="col-md-10">
                        <select class="form-control" v-model="data.has_registered_on_website"
                                id="has_registered_on_website">
                            <option value="true">Yes</option>
                            <option value="false">No</option>
                        </select>
                        <span class="help-block is-danger"><strong>{{ errors.get('has_registered_on_website')
                            }}</strong></span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <div class="col-md-offset-2 col-md-10">
                        <button type="button" class="btn btn-w-m btn-primary" @click.prevent="postPut()">Update</button>
...
<script>
    export default {
        ...
        methods: {
            postPut() {
                this.myAxios.put(this.ridersResourceUrl + '/' + this.$route.params.id, this.data)
                    .then((response) => {
                        this.showElementNotify('Rider Updated', 'Rider has updated.');
            ...
            getRider() {
                this.myAxios.get(this.ridersResourceUrl + '/' + this.$route.params.id)
                    .then((response) => {
                        this.data = response.data.rider;
                    })
                    .catch((error) => {
                        console.log(error);
                        this.showErrorNotify(error);
